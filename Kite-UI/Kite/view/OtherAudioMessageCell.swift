...
class OtherAudioMessageCell: UITableViewCell, AVAudioPlayerDelegate, TimestampRevealable {
    
    let bubble = UIView()
    let button = UIView()
    let buttonImg = UIImageView()
    let playImage = UIImage(systemName: "play.fill")?.withTintColor(UIColor.white, renderingMode: .alwaysOriginal)
    let pauseImage = UIImage(systemName: "pause.fill")?.withTintColor(UIColor.white, renderingMode: .alwaysOriginal)
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        selectionStyle = .none
        bubble.leftAnchor.constraint(equalTo: contentView.leftAnchor, constant: 20).isActive = true
        ...
        bubble.layer.maskedCorners = [.layerMaxXMinYCorner, .layerMinXMinYCorner, .layerMaxXMaxYCorner]
        bubble.backgroundColor = UIColor(red: 0.851, green: 0.851, blue: 0.851, alpha: 1)
        ...
        timeLabel.leftAnchor.constraint(equalTo: bubble.leftAnchor, constant: 15).isActive = true
        ...
        timeLabel.rightAnchor.constraint(equalTo: button.leftAnchor, constant: -15).isActive = true
        ...
        bubble.rightAnchor.constraint(equalTo: button.rightAnchor, constant: -15).isActive = true
...
//    func setAudio(audio: Data) {
//        self.audio = audio
//        let audioFileName = UUID().uuidString + ".m4a"
//        contentURL = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent(audioFileName)
//
//        print("UserAudioMessageCall: setAudio audio des:")
//        do {
//            try audio.write(to: contentURL!)
//            try AVAudioSession.sharedInstance().setCategory(.playback)
//            try AVAudioSession.sharedInstance().setActive(true)
//            try audioPlayer = AVAudioPlayer(contentsOf: contentURL!)
//            audioPlayer?.prepareToPlay()
//            audioPlayer?.delegate = self
//        } catch {
//            print("UserAudioMessageCell: AudioPlayer did not setup \(error.localizedDescription)")
